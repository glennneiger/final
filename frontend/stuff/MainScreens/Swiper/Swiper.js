...
import { modalStore } from "../../reduxStuff";
...
export default class Swiper extends Component {
  ...
  flooby = new Animated.Value(0);
  changeFlooby(change) {
    this.flooby = new Animated.Value(change);
  }
  animateFlooby(newFlooby) {
    if (newFlooby != 0) {
      modalStore.dispatch({ type: "nus", payload: true });
    }
    Animated.timing(this.flooby, { toValue: newFlooby, duration: 250 }).start();
  }
  ...
  onPanResponderMove = (event, gestureState) => {
    if (gestureState.dy > 0) {
      if (this.state.og !== 0) {
        this.setState({ og: gestureState.dy, allowed: false });
      }
      this.changeFlooby((gestureState.dy * 2) / 3);
    }
    this.props.onSwiping(this._animatedValueX);
  ...
  onPanResponderRelease = (e, gestureState) => {
    this.setState({ allowed: true, og: 0 });
    const newFlooby = gestureState.dy > 150 ? height : 0;
    this.animateFlooby(newFlooby);
    if (this.state.panResponderLocked) {
      this.state.pan.setValue({
        x: 0,
        y: 0
      ...
      <Animated.View
        style={[
          styles.container,
          {
            ...
            transform: [{ translateY: this.flooby }]
          },
